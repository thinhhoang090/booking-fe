...
function PublicBookingRoom() {
  ...
  useEffect(() => {
    ...
    const getCategory = async () => {
      var result = await getMethod(
        "http://localhost:8080/api/category/public/findAll?type=ROOM"
      );
      setCategory(result);
    };
    getCategory();
  ...
  async function loadAllRoom() {
    ...
    var url =
      "http://localhost:8080/api/booking/public/cal-date?from=" +
      from +
      "&numDate=" +
      songay;
    if (cateId != -1) {
    ...
    setItems(list);
    document.getElementById("sophongtrong").innerHTML =
      "(" + list.length + " phòng)";
  ...
  function loadRoomSelect() {
    ...
    for (var i = 0; i < listRoomArr.length; i++) {
      main += `<div class="row">
            ...
            <div class="col-3"><span>${formatMoney(
              listRoomArr[i].price
            )}</span></div>
        </div>`;
      try {
        document.getElementById(
          "inputcheck" + listRoomArr[i].id
        ).checked = true;
      } catch (error) {}
    }
    ...
    <div class="contentmain">
      <section>
          ...
          <div class="blockbookingtime">
            <div class="row">
              ...
              <div class="contenttimebk col-sm-4">
                ...
                <span id="ngaytraphong" class="ngaytraphong">
                  Thứ 5, 16 thg 1, 2023
                </span>
              </div>
              <div class="divbtnsearchrooms">
                <button onClick={() => loadAllRoom()} class="btnsearchroom">
        ...
        <div class="row">
          <div class="col-sm-8">
            <p class="titlept">
              Danh sách phòng trống <span id="sophongtrong"></span>
            </p>
                  ...
                  <div class="row singleroom">
                    ...
                    <div class="col-8">
                      ...
                      <div class="d-flex numbedroom">
                        <span>
                          <i class="fa fa-bed"></i> Số giường: {item.namenumBed}
                        </span>
                        <span>
                          <i class="fa fa-users"></i> Số người tối đa:{" "}
                          {item.maxPeople}
                        </span>
                      </div>
                      <div class="row">
                            ...
                            {item.roomUtilities.map((tienich) => {
                              return (
                                <div class="col-sm-6 singletich">
                                  <i class={tienich.utilities.icon}></i>{" "}
                                  {tienich.utilities.name}
                                </div>
                              );
                            })}
                          </div>
                        ...
                        <div class="col-4">
                          ...
                          <label class="checkbox-custom">
                            ...
                            Chọn phòng
                            <input
                              id={"inputcheck" + item.id}
                              onChange={() =>
                                pushToList(item.id, item.name, item.price)
                              }
                              type="checkbox"
                            />
          ...
          <div class="col-sm-4 toppad">
            <div class="timebooking row">
              <div class="headertimebk">Phòng đã chọn</div>
            ...
            <div
                  ...
                  {room?.roomImages.map((img , index) => {
                    return (
                      <div class={`carousel-item ${index === 0 ? "active" : ""  }`}  >
                        <img src={img.image} width={500} height={350}  class="d-block w-full" alt="..." />
                      </div>
                    );
                  })}
                </div>
              ...
              <div
                dangerouslySetInnerHTML={{
                  __html: room == null ? "" : room.description,
                }}
              ></div>
            </div>
